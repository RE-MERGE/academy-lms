...
<div style="font-family: sans-serif; padding: 10px 0 20px 0;">
    ...
    <!-- ✅ 수정1: 상단 필터 버튼 - keyword 파라미터 유지 -->
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px;">
        <div style="display: flex; align-items: center; gap: 8px;">
            <button class="filter-btn" id="btn-all"
                    onclick="location.href='?page=1&role=all'">전체</button>
            <button class="filter-btn" id="btn-student"
                    onclick="location.href='?page=1&role=student'">학생</button>
            <button class="filter-btn" id="btn-professor"
                    onclick="location.href='?page=1&role=professor'">교수</button>
            <button class="filter-btn" id="btn-admin"
                    onclick="location.href='?page=1&role=admin'">관리자</button>
        </div>

        <!-- ✅ 수정2: 검색창 - form GET으로 서버에 전송 -->
        <form method="get" action="${pageContext.request.contextPath}/admin/userList"
              class="search-wrap" style="margin:0;">
            <input type="hidden" name="role" value="${currentRole}"/>
            <input type="hidden" name="page" value="1"/>

            <select name="searchType">
                <option value="name"      ${searchType eq 'name'      ? 'selected' : ''}>이름</option>
                <option value="userCode"  ${searchType eq 'userCode'  ? 'selected' : ''}>학번</option>
                <option value="email"     ${searchType eq 'email'     ? 'selected' : ''}>이메일</option>
                <option value="nameEmail" ${searchType eq 'nameEmail' ? 'selected' : ''}>이름+이메일</option>
                <option value="all"       ${searchType eq 'all'       ? 'selected' : ''}>이름+이메일+학번</option>
            </select>
    ...
    <table class="user-table" id="userTable">
        ...
        <c:forEach var="user" items="${userList}" varStatus="status">
            <tr class="user-row"
                ...
                <td>
                    <c:choose>
                    <c:when test="${user.role eq 'STUDENT'}">학생</c:when>
                    <c:when test="${user.role eq 'PROFESSOR'}">교수</c:when>
