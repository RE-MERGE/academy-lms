...
<head>
    ...
    <style>
        ...
        .grade-table td:nth-child(2) { width: 25%; }

        .grade-table th:nth-child(3),
        .grade-table td:nth-child(3) { width: 25%; }

        .grade-table th:nth-child(4),
        .grade-table td:nth-child(4) { width: 25%; }
                        ...
                        <div class="course-card">
                            <c:choose>
                                ...
                                <c:when test="${course.course_type eq 'MAJOR_ELECTIVE'}">
                                    <c:set var="badgeText" value="전공선택"/><c:set var="badgeClass" value="badge-green"/>
                                </c:when>
                                <c:when test="${course.course_type eq 'GENERAL_REQUIRED'}">
                                    <c:set var="badgeText" value="일반필수"/><c:set var="badgeClass" value="badge-yellow"/>
                                </c:when>
                                <c:otherwise>
                                    <c:set var="badgeText" value="일반선택"/><c:set var="badgeClass" value="badge-blue"/>
                                </c:otherwise>
                            </c:choose>
...
<script>
    ...
    function renderMpTimetable() {
        ...
        /* 색상 매핑 */
        var colorMap = {};
        mp_courses.forEach((c, i) => { colorMap[c.course_no] = MP_COLORS[i % MP_COLORS.length]; });

        /* 시간 범위 계산 */
        var minH = 9, maxH = 18;
        mp_courses.forEach(c => {
            var sh = mpParseHour(c.start_time), eh = mpParseHour(c.end_time);
        ...
        for (var h = minH; h < maxH; h++) {
            ...
            DAY_KEYS.forEach((day, di) => {
                var occupied = mp_courses.some(c => {
                    var days = mpParseDays(c.day_of_week);
                    var sh = mpParseHour(c.start_time), eh = mpParseHour(c.end_time);
                    return days.includes(day) && sh <= h && h < eh;
                });
                if (!occupied) {
    ...
    function loadMpTimetable() {
        ...
        fetch(endpoint, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(r => r.json())
            .then(data => { mp_courses = data.courses || []; renderMpTimetable(); })
            .catch(() => { mp_courses = []; renderMpTimetable(); });
    }
